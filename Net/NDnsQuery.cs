...
public class NDnsQuery
{
    ...
    private static extern int DnsQuery(string pszName, QueryTypes wType, QueryOptions options, int aipServers, ref IntPtr ppQueryResults, int pReserved);
    ...
    public static List<string> GetSRVRecords(string needle)
    {
        ...
        try
        {
            ...
            {
                // This cast is not needed in C#
                var recSRV = (SRVRecord)Marshal.PtrToStructure(ptr2, typeof(SRVRecord));

                if (recSRV.wType == (ushort)QueryTypes.DNS_TYPE_SRV)
                {
                    string targetIp = Marshal.PtrToStringUni(recSRV.pNameTarget);
                    string targetPort = recSRV.wPort.ToString();
